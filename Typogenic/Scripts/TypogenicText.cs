...
public class TypogenicText : MonoBehaviour
{
    ...
    public bool JustifiedStrict = false;
    ...
    public void RebuildMesh()
    {
        ...
        if (WordWrap < Mathf.Epsilon)
        {
            foreach (string line in lines)
            {
                if (Alignment == TTextAlignment.Center)
                {
                    cursorX = -GetStringWidth(line) * 0.50f;
                }
                else
                ...
                cursorY = cursorY + Font.LineHeight * Size + Leading + ParagraphSpacing;
            }
        }
        else
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (Alignment == TTextAlignment.Justified)
                {
                    List<List<string>>  wordsPerline    = new List<List<string>>();
                    List<string>        wordsCurrent    = new List<string>();
                    List<float>         wordsPadding    = new List<float>();

                    float s = 0.00f;
                    float n = 0.00f;

                    /*
                     *  First we need to figure out how the different words need
                     *  to be split into different lines much like we would have
                     *  done for any other alignment but we also need to get the
                     *  lengths of each of these lines, to use with padding math
                     */
                    foreach (string word in line.Split(' '))
                    {
                        if (s + GetStringWidth(word) > WordWrap)
                        {
                            wordsPerline.Add(wordsCurrent);
                            wordsPadding.Add((WordWrap - s) / (n - 1.00f));
                            wordsCurrent = new List<string>
                            {
                                word
                            };

                            s = GetStringWidth(word + " ");
                            n = 1.00f;
                        }
                        else
                        {
                            wordsCurrent.Add(word);

                            s = s + GetStringWidth(word + " ");
                            n = n + 1.00f;
                        }
                    }

                    /*
                     *  Handle the last line as well. If you wanted to do a full
                     *  text justified alignment, this line should be changed to
                     *  add some padding to the last line. But yeah _if_ that is
                     */
                    wordsPerline.Add(wordsCurrent);

                    if (JustifiedStrict && n > 1)
                    {
                        wordsPadding.Add((WordWrap - s) / (n - 1.00f));
                    }
                    else
                    {
                        wordsPadding.Add(0.00f);
                    }

                    /*
                     *  Iterate through, each of the lines, then built them just
                     *  like we would have done normally by adding the linebreak
                     */
                    for (int i = 0; i < wordsPerline.Count; i++)
                    {
                        foreach (string word in wordsPerline[i])
                        {
                            cursorX = BlitString(word + " ", cursorX, cursorY, vertexPointers) + wordsPadding[i];
                        }
                        ...
                        cursorX = 0.00f;
                        cursorY = cursorY + Font.LineHeight * Size + Leading;
                    }

                    cursorY = cursorY + ParagraphSpacing;
                }
                else
                {
                    /*
                     *  This is the old code, and is run if the alignment is not
                     *  justified. I re-wrote it a bit, but the basic runtime of
                     *  it has not changed. Note the if check is done as were it
                     *  left alignment, as the size is the same for both concats
                     */
                    foreach (string word in line.Split(' '))
                    {
                        if (cursorX + GetStringWidth(word + " ") > WordWrap)
                        {
                            OffsetStringPosition(vertexPointers, cursorX);
                            vertexPointers.Clear();
                        ...
                        cursorX = BlitString((Alignment == TTextAlignment.Right ? word + " " : " " + word), cursorX, cursorY, vertexPointers);
                    }
